...
def Extract(prod_path, prod_pct_path, mean_pct_path, asset_path, hedge_path):
    ...
    df_hedge: DataFrame
        hedge dataframe
    '''
    df_prod=ReadExcelFile(prod_path, sheet_name="prod")
    df_prod_pct=ReadExcelFile(prod_pct_path, sheet_name="prod_perc")
    df_mean_pct=ReadExcelFile(mean_pct_path, sheet_name="mean_perc")
...
df_prod, df_prod_pct, df_mean_pct, df_asset, df_hedge = Extract(prod_path=prod, prod_pct_path=prod, 
...
hedge_vmr_planif=TransformHedge(data_prod=df_prod, hedge=df_hedge, 
                                prod_pct=df_prod_pct, mean_pct=df_mean_pct, 
                                oa=df_oa, cr=df_cr, ppa=df_ppa, profile=df_prod_pct)
...
Load(dest_dir=dest_dir, src_flow=hedge_vmr_planif, file_name="hedge_vmr_planif")
